...
{	
	...
	this->weight = new warthog::dbword[header_height()*header_width()];
	for(unsigned int i=0; i < header_height()*header_width(); i++)
	{
		weight[i] = 0;
	}
...
warthog::gridmap::gridmap(const char* filename)
{
	...
	// unsigned int boom_cost = 55;
	// this->set_label(to_padded_id(seed), boom_cost);
	// this->set_label(to_padded_id(seed)+1, boom_cost); 	//east
	// this->set_label(to_padded_id(seed)-1, boom_cost);	//west
	// this->set_label(to_padded_id(seed)+ this->padded_width_, boom_cost);	//south
	// this->set_label(to_padded_id(seed)- this->padded_width_, boom_cost);	//north
	// this->set_label(to_padded_id(seed)+ this->padded_width_ +1, boom_cost);//SE
	// this->set_label(to_padded_id(seed)+ this->padded_width_ -1, boom_cost);//SW
	// this->set_label(to_padded_id(seed)- this->padded_width_ +1, boom_cost);//NE
	// this->set_label(to_padded_id(seed)- this->padded_width_ -1, boom_cost);//NW
}
