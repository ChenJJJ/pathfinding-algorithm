...
class weighted_gridmap
{
	public:
		...
		get_neighbours(uint32_t db_id, uint32_t ids[9], warthog::dbword costs[9])
		{
            ...
            costs[6] = db_[ids[6]];
            costs[7] = db_[ids[7]];
            costs[8] = db_[ids[8]];      //store the corresponding char, i.e if our ASCII value is 55, hear we read '7',
										//this is because we define the cost as dbword(unsigned char defined in constant.h)
		}

		inline void
		bomb(unsigned int seed,unsigned int radius, unsigned int bomb_cost)
		{
			
			this->set_label(to_padded_id(seed), (this->get_label(to_padded_id(seed)) < bomb_cost)? bomb_cost : this->get_label(to_padded_id(seed))) ;
			this->set_label(to_padded_id(seed)+1, (this->get_label(to_padded_id(seed)+1) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)+1)); 	//east
			this->set_label(to_padded_id(seed)-1, (this->get_label(to_padded_id(seed)-1) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)-1));	//west
			this->set_label(to_padded_id(seed)+ this->padded_width_, (this->get_label(to_padded_id(seed)+ this->padded_width_) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)+ this->padded_width_));	//south
			this->set_label(to_padded_id(seed)- this->padded_width_, (this->get_label(to_padded_id(seed)- this->padded_width_) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)- this->padded_width_));	//north
			this->set_label(to_padded_id(seed)+ this->padded_width_ +1, (this->get_label(to_padded_id(seed)+ this->padded_width_+1) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)+ this->padded_width_+1));//SE
			this->set_label(to_padded_id(seed)+ this->padded_width_ -1, (this->get_label(to_padded_id(seed)+ this->padded_width_-1) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)+ this->padded_width_-1));//SW
			this->set_label(to_padded_id(seed)- this->padded_width_ +1, (this->get_label(to_padded_id(seed)- this->padded_width_ +1) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)- this->padded_width_+1));//NE
			this->set_label(to_padded_id(seed)- this->padded_width_ -1, (this->get_label(to_padded_id(seed)- this->padded_width_ -1) < bomb_cost) ? bomb_cost : this->get_label(to_padded_id(seed)- this->padded_width_-1));//NW
		}
