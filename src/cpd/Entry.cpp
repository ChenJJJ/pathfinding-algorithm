...
void warthog::PreprocessMap(std::vector<bool> &bits, int width, int height, const char *filename)
{
	...
	CPD cpd;
	{
		...
		
		{
			...
			//printf("Estimated sequential running time : %dmin\n", static_cast<int>((t.warthog::timer::elapsed_time_nano()/1000000000)*g.node_count()/60.0));
		}
...
void *warthog::PrepareForSearch(std::vector<bool> &bits, int w, int h, const char *filename)
{
...
bool warthog::GetPath(void *data, xyLoc s, xyLoc t, std::vector<xyLoc> &path)
{
	...
	if(path.empty()){
		...
		if(state->current_node != state->target_node){
			path.vector::push_back(state->mapper(state->current_node));
...
bool warthog::GetPath(void *data, xyLoc s, xyLoc t, std::vector<xyLoc> &path)
{
	...
	if(first_move == 0xF)
		return true;
	for(;;){
		path.vector::push_back(state->mapper(current_node));
