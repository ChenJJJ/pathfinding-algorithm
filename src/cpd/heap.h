...
public:
	...
	void reset(int new_id_count = 0, key_order_type new_order = key_order_type()){
		heap.vector::resize(new_id_count);
		id_pos.vector::resize(new_id_count);
		clear();
	...
	bool contains(int id)const{
		assert(0 <= id && id < (int)id_pos.vector::size() && "id is in range");
...
private:
	...
	void check_id_invariants()const{
		...
		for(int i=0; i<heap_end; ++i){
			assert(heap[i].id != -1);
			assert(0 <= heap[i].id);
			assert(heap[i].id < (int)id_pos.vector::size());
			assert(id_pos[heap[i].id] == i);
		}

		for(int i=0; i<(int)id_pos.vector::size(); ++i){
			if(id_pos[i] != -1){
				assert(0 <= id_pos[i]);
