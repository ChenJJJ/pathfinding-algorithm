...
ListGraph extract_node_induced_subgraph(
	...
	assert(g.is_valid());
	assert(g.node_count() == (int)g_to_top_level.vector::size());
	int s_node_count = 0;
	...
	s_to_top_level.vector::resize(s_node_count);

	g_to_s.vector::resize(g.node_count());
	...
	for(auto a:g.arc)
		if(g_to_s[a.source] != -1 && g_to_s[a.target] != -1)
			s.arc.vector::push_back({g_to_s[a.source], g_to_s[a.target]});
