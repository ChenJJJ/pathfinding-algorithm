...
void CPD::append_row(int source_node, const std::vector<unsigned short>&allowed_first_move){
	...
	for(int i=1; i<(int)allowed_first_move.vector::size(); ++i){
		int allowed_next = allowed_up_to_now & get_allowed(i);
		if(allowed_next == 0){
			entry.vector::push_back((node_begin << 4) | find_first_allowed_out_arc(allowed_up_to_now));
			node_begin = i;
	...
	begin.vector::push_back(entry.vector::size());


	#ifndef NDEBUG
	int pos = 0;
	const int node_count = allowed_first_move.vector::size();
...
void CPD::append_rows(const CPD&other){
	...
	for(auto x:make_range(other.begin.begin()+1, other.begin.end()))
		begin.vector::push_back(x + offset);
