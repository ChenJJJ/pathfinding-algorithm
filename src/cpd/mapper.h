		...
		pos_to_node_(width*height, -1)
	{
		for(int y=0; y<height_; ++y)
			for(int x=0; x<width_; ++x)
				if(field[x+y*width_]){
					node_to_pos_.vector::push_back({static_cast<std::int16_t>(x), static_cast<std::int16_t>(y)});
	...
	for(int u=0; u<mapper.node_count(); ++u){
		...
		for(int d = 0; d<8; ++d){
			...
			if(v != -1 && mapper(xyLoc{u_pos.x, static_cast<std::int16_t>(u_pos.y+dy[d])}) != -1 &&  mapper(xyLoc{static_cast<std::int16_t>(u_pos.x+dx[d]), u_pos.y}) != -1)
				// g.arc.vector::push_back({u, v, dw[d]* (map->get_label(map->to_padded_id(u_pos.x,u_pos.y)))});
				g.arc.vector::push_back({u, v, dw[d]});
		}
	}
	return g;
}
