...
class flexible_astar 
{
	public:
		...
		inline std::stack<uint32_t>
		get_path_cpd(uint32_t startid, uint32_t goalid, void *reference)
		{
			std::stack<uint32_t> path;
			warthog::search_node* goal = search_cpd(startid, goalid, reference);
			if(goal)
			{
				// follow backpointers to extract the path
				assert(goal->get_id() == goalid);
		...
		double
		get_length_cpd(uint32_t startid, uint32_t goalid, void *reference, std::vector<double> &h_array,std::vector<int> &exp_id,  int current_id)
		{
			warthog::search_node* goal = search_cpd(startid, goalid, reference, h_array, exp_id, current_id);
			warthog::cost_t len = warthog::INF;
			if(goal)
			{
				assert(goal->get_id() == goalid);
	...
	private:
		...
		warthog::search_node*
		search_cpd(uint32_t startid, uint32_t goalid, void *reference,std::vector<double> &h_array, std::vector<int> &exp_id, int current_id)
		{
			...
			start->set_f(heuristic_->h_cpd(startid, goalid, reference, h_array, exp_id, current_id) * hscale_);
			open_->push(start);
			...
			while(open_->size())
			{
				...
				//test to see the unpadded x y to check the visual path in map
				// if(cccc == 1){
				// uint32_t x, y;
				// // x= current->get_id(); 
				// // y=(x-3*64)/64;
				// // z=(x-3*64)%64;
				// int starty = ((startid)- 2 * (expander_->mapwidth())) / expander_->mapwidth();
				// int startx = ((startid)- 2* (expander_->mapwidth())) % expander_->mapwidth();
				// int goaly = ((goalid)- 2 * (expander_->mapwidth())) / expander_->mapwidth();
				// int goalx = ((goalid)- 2* (expander_->mapwidth())) % expander_->mapwidth();
				
				// y = ((current->get_id())- 2 * (expander_->mapwidth())) / expander_->mapwidth();
				// x = ((current->get_id())- 2* (expander_->mapwidth())) % expander_->mapwidth();
				// std::cout<<"start"<<startx<<","<<starty<<"\t"<<"goal"<<goalx<<","<<goaly<<"\t"<<x<<","<<y<<std::endl;
				// }
				#ifndef NDEBUG
				if(verbose_)
				{
				...
				//NB: the g cost of moving is store from N and clockwisely in 8 byte array
					   	...
					   	expander_->next(n, cost_to_n))
				{
					...
					else
					{
						...
						warthog::cost_t gval = current->get_g() + cost_to_n;
						n->set_g(gval);
						n->set_f(gval + heuristic_->h_cpd(n->get_id(), goalid, reference, h_array, exp_id, current_id) * hscale_);
